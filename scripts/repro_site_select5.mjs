...
await page.evaluate(() => {
  const tabs = Array.from(document.querySelectorAll('button.tab-list__item'));
  const analysisTab = tabs.find((btn) => btn.textContent?.includes('分析'));
  analysisTab?.click();
});
...
await page.evaluate(() => {
  const select = document.querySelector('#site-group-select');
  if (select) {
...
await page.evaluate(() => {
  ...
  const bboxBtn = buttons.find((btn) => btn.textContent?.trim() === '框选范围');
  bboxBtn?.click();
});
...
await page.evaluate(() => {
  const buttons = Array.from(document.querySelectorAll('button'));
  const startBtn = buttons.find((btn) => btn.textContent?.trim() === '开始选址');
  startBtn?.click();
});
